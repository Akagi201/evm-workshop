...
impl ForkDB {
    pub fn new<B: Into<u64>>(rpc_url: Url, block_number: B) -> Self {
        let http = Http::<Client>::new(rpc_url);
        let provider = ProviderBuilder::new().provider(RootProvider::new(RpcClient::new(http, false)));
        ForkDB {
            provider,
    ...
    pub fn new_as_cache_db<B: Into<u64>>(
        rpc_url: Url,
        block_number: B,
    ) -> CacheDB<Self> {
...
impl DatabaseRef for ForkDB {
    ...
    fn basic_ref(&self, address: Address) -> Result<Option<AccountInfo>, Self::Error> {
        ...
        let balance = self.provider.get_balance(address, Some(self.block_number));
